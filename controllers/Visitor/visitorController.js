...
export const sendNoti = async (content, id, isVisitor, status) => {
  ...
  axios
    .post("https://onesignal.com/api/v1/notifications", data, { headers })
    .then((response) => {
...
export const AddVisitor = bigPromise(async (req, res, next) => {
  ...
  if (news_letter_confirmation == 1) {
    sendMail(
      email,
      "2Gethr News Letter Confirmation",
      newsLetter(name, email, phone, company_name)
    );
  }
  ...
  await Visitor.create(obj).then(async (data) => {
    try {
      if (contact_person_email) {
        sendNoti(
          `"${name}" has been Pre-Registered for ${
            pre_register[0].session_start_date.split("T")[0]
          } and invite has been sent to email.`,
          req.body.user_id,
          true,
          "UPCOMING"
        );
        ...
        sendNoti(
          `"${name}" has requested for the Check In. Please Accept/Reject.`,
          req.body.user_id,
          true,
          "UPCOMING"
        );
...
export const getAllVisitors = bigPromise(async (req, res, next) => {
  ...
  await Visitor.find(obj)
    .then((data) => {
      if (data.length > 0) {
        data?.sort(
          (a, b) =>
            new Date(
              b.pre_register[b.pre_register.length - 1]?.session_start_date
            ) -
            new Date(
              a.pre_register[a.pre_register.length - 1]?.session_start_date
            )
        );
      }
...
export const updateVisitor = bigPromise(async (req, res, next) => {
  ...
  if (req.body.pre_register?.length > 0) {
    let x6 = repData?.pre_register?.filter(
      (s) =>
        ...
        new Date(s.session_start_date).toISOString() ==
          new Date(req.body.pre_register[0]?.session_start_date).toISOString()
    );
    if (x6?.length > 0) {
      return res.status(200).json({
        success: false,
  ...
  if (
    req.body.news_letter_confirmation == 1 ||
    req.body.news_letter_confirmation == 0
  ) {
  ...
  if (req.body.pre_register) {
    ...
    obj.pre_register = q;
    sendNoti(
      `"${visitor?.name}" has been Pre-Registered for ${
        req.body.pre_register[0]?.session_start_date?.split("T")[0]
      } and invite has been sent to email.`,
      req.body.pre_register[0]?.user_id,
      true,
      "UPCOMING"
    );
  }
  if (qr_code) {
    if (req.body.id) {
      let pre = visitor?.pre_register?.filter(
        (s) => s.id == parseInt(req.body.id)
      );
      pre[0].qr_code = qr_code;
      let pre1 = visitor?.pre_register?.filter(
        (s) => s.id != parseInt(req.body.id)
      );
      pre1.push(pre[0]);
    ...
    sendMail(
      visitor.email,
      "2Gethr - Visitor Pre-Register Confirmation",
      mailTemp(qr_code)
    );
  }
  if (req.query.log_id) {
  ...
  await Visitor.findOneAndUpdate(
    ...
    .then(async (data) => {
      ...
      if (image && req.body.user_id && req.body.qr) {
        sendNoti(
          `"${visitor?.name}" has Scanned Visitor QR and Just Checked In to the facility.`,
          req.body.user_id,
          true,
          "ACTIVE"
        );
      ...
      if (image && req.body.user_id && !req.body.qr) {
        sendNoti(
          `"${visitor?.name}" has requested for the Check In, Please Accept/Reject.`,
          req.body.user_id,
          true,
          "UPCOMING"
        );
      ...
      if (req.body.pre_register) {
        ...
        data1.location = pre_reg[0]?.location;
      }
      return res.status(200).json({
        success: true,
        data: data1,
...
export const bulkLogoutVisitors = bigPromise(async (req, res, next) => {
  ...
  for (let i of data) {
    ...
    await Visitor.updateOne(
      {
        _id: i?.id,
        "pre_register.id": i?.pre_register_id,
        "pre_register.status": "ACTIVE",
      },
      {
        $set: {
...
export const HistoryVisitors = async (req, res, next) => {
    ...
    .then(async (data) => {
      for (let i of data) {
        ...
        if (i.status == "CHECKEDOUT") {
          data2 = i?.logs?.filter(
            (s) =>
              s.company_name == req.query.visiting_company_name &&
              s.user_id == req.query.user_id &&
              (s.status == "INACTIVE" ||
                s.status == "ACTIVE" ||
                (s.status == "UPCOMING" &&
                  s.in_time.toISOString().split("T")[0] < today.split("T")[0]))
          );
        } else {
          data2 = i?.logs?.filter(
            (s) =>
  ...
  await Visitor.find({
    ...
    .then(async (data) => {
      console.log(data);
      for (let i of data) {
        let data3 = i?.pre_register?.filter(
          (s) =>
            s.company_name == req.query.visiting_company_name &&
            (s.status == "INACTIVE" ||
              (s.status == "UPCOMING" &&
                s.session_start_date.toISOString().split("T")[0] <
                  today.split("T")[0])) &&
            s.user_id == req.query.user_id
        );
        i.logs = [];
...
export const CountVisitors = async (req, res, next) => {
    ...
    $or: [
      ...
      },
      {
        $and: [
          ...
          { "logs.in_time": { $lt: today.split("T")[0] } }, // Check if in_time is less than today
        ],
      },
    ],
  }).then(async (data) => {
    ...
    $or: [
      ...
      },
      {
        $and: [
          ...
          { "pre_register.user_id": req.query.user_id },
          { "pre_register.status": "UPCOMING" },
          { "pre_register.session_start_date": { $lt: today.split("T")[0] } },
        ],
      },
    ],
  }).then(async (data) => {
    for (let i of data) {
      let data2 = i?.pre_register?.filter(
        (s) =>
          s.company_name == req.query.visiting_company_name &&
          (s.status == "INACTIVE" ||
            (s.status == "UPCOMING" &&
  ...
  await Visitor.find({
    $and: [
      ...
      { "logs.status": "UPCOMING" },
      { "logs.user_id": req.query.user_id },
      { "logs.in_time": { $gte: today.split("T")[0] } },
    ],
    raw: true,
  }).then(async (data) => {
    cnt = 0;
  ...
  await Visitor.find({
    $and: [
      ...
      { "pre_register.user_id": req.query.user_id },
      { "pre_register.status": "UPCOMING" },
      { "pre_register.session_start_date": { $gte: today.split("T")[0] } },
    ],
    raw: true,
  }).then(async (data) => {
    for (let i of data) {
...
export const getCompanyName = async (req, res, next) => {
  ...
  console.log(name);
  await User.findOne({ organisationName: name })
    .then((data) => {
      return res.status(200).json({
        sucess: true,
        data: data,
...
export const updateTemplate = bigPromise(async (req, res, next) => {
  ...
  await sendGridMailTemplate
    .findByIdAndUpdate({ _id: old_id }, { $set: { is_selected: false } })
    .catch((err) => {
      console.log(err);
      logger.info(err);
      return res.status(401).json({
  ...
  await sendGridMailTemplate
    .findByIdAndUpdate({ _id: new_id }, { $set: { is_selected: true } })
    .then(async (data) => {
      let data1 = await sendGridMailTemplate
...
export const getUpcoming = bigPromise(async (req, res) => {
  ...
  const filteredData = data.filter((record) => {
    ...
    return (
      ...
      sessionStartDate.toDateString() === today.toDateString()
    );
  });
  return res.status(200).json({
    success: true,
...
//     apiKey: "41e1d7e2",
//     apiSecret: "av6AWGZT8F4VmkoQ",
//   });

//   const from = "Vonage APIs";
//   const to = phone;
//   const text = `Use OTP ${otp} for verification.`;
